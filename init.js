...
function calculate_distribution(real,predict){
  ...
  option = {
    ...
    legend: {
      data:['Реальные значения','Прогнозируемые значения']
    },
    xAxis: [
      {
        type: 'category',
...
function plot_stock(){
  ...
  option = {
    ...
    legend: {
      top: 30,
      data: ['Рынок', 'Скользащая средняя 5 дней', 'Скользащая средняя 10 дней', 'Скользащая средняя 20 дней', 'Скользащая средняя 30 дней']
    },
    tooltip: {
      trigger: 'axis',
    ...
    series: [{
      ...
      name: 'Скользащая средняя 5 дней',
      type: 'line',
      data: dataMA5,
      smooth: true,
      showSymbol: false,
      ...
      name: 'Скользащая средняя 10 дней',
      type: 'line',
      data: dataMA10,
      smooth: true,
      showSymbol: false,
      ...
      name: 'Скользащая средняя 20 дней',
      type: 'line',
      data: dataMA20,
      smooth: true,
      showSymbol: false,
    ...
    {
      name: 'Скользащая средняя 30 дней',
      type: 'line',
      data: dataMA30,
      smooth: true,
      showSymbol: false,
...
$('#trainbutton').click(function(){
  ...
  if(parseFloat($('#inputdropoutrate').val())<0 || parseFloat($('#inputdropoutrate').val())>1){
    Materialize.toast('Входящие выпадающие значения должны быть больше 0 и меньше 1', 4000)
    return
  }
  if(parseFloat($('#smooth').val())<0 || parseFloat($('#smooth').val())>1){
    Materialize.toast('Веса для сглаживания должны быть больше 0 и меньше 1', 4000)
    return
  }
  if(parseFloat($('#outputdropoutrate').val())<0 || parseFloat($('#outputdropoutrate').val())>1){
    Materialize.toast('Выходящие выпадающие значения должны быть больше 0 и меньше 1', 4000)
    return
  }
  setTimeout(function(){
    ...
    function async_training_loop(callback) {
      (function loop(i) {
        ...
        $('#log').append('Итерация: '+(i+1)+', Средняя потеря: '+total_loss+'<br>');
        ...
        $('#div_output').attr('style','height:550px;');
        ...
        option = {
          ...
          legend: {
            top: 30,
            data: ['Рынок', 'Скользащая средняя 5 дней', 'Скользащая средняя 10 дней', 'Скользащая средняя 20 дней', 'Скользащая средняя 30 дней','Прогнозируемая цена закрытия']
          },
          tooltip: {
            trigger: 'axis',
            position: function (pt) {
          ...
          {
            name: 'Прогнозирумая цена закрытия',
            type: 'line',
            data: predicted_val,
            smooth: false,
            showSymbol: false,
    ...
    async_training_loop(function() {
      ...
      $('#log-invest').append("<h6 class='header'>Суммарный заработок: "+ my_investment['overall gain']+", Суммарный % инвестиций: "+my_investment['overall investment']+"%</h5>")
...
async_training_loop(function() {
  ...
  stock_changes_percent = (stock_changes / close[0])*100
      ...
      for (var i = 0; i < my_investment['buy_X'].length;i++){
        ...
        markpoints.push({name: 'buy', value: 'Покупка', xAxis: ind, yAxis: my_investment['buy_Y'][i],itemStyle:{color:'#61a0a8'}})
      }
      for (var i = 0; i < my_investment['sell_X'].length;i++){
        ind = new_date.indexOf(my_investment['sell_X'][i])
        markpoints.push({name: 'sell', value: 'Продажа', xAxis: ind, yAxis: my_investment['sell_Y'][i],itemStyle:{color:'#c23531'}})
      }
      option = {
        animation: false,
        color: color_list,
        ...
        legend: {
          top: 30,
          data: ['Рынок', 
          'Скользащая средняя 5 дней', 
          'Скользащая средняя 10 дней', 
          'Скользащая средняя 20 дней', 
          'Скользащая средняя 30 дней',
          'Прогнозируемая цена закрытия','Продажа','Покупка']
        },
        tooltip: {
          trigger: 'axis',
          position: function (pt) {
        ...
        {
          name: 'Прогнозируемая цена закрытия',
          type: 'line',
          data: predicted_val,
          smooth: false,
          showSymbol: false,
...
function plot_pairplot(val){
  ...
  generate_investment(['total investment(%): ','total gains: ','Рынок changes: ','Рынок changes (%): ','gold changes(%): ','crude oil changes(%): '],
